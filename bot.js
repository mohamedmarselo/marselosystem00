...
if (!message.content.startsWith(prefix)) return;
	...
	if (command == "mute") {
		...
		if (message.mentions.users.size < 1) return message.reply('**منشن ل الي هتعملو ميوت **').then(msg => {msg.delete(5000)});
...
if(!message.guild.member(client.user).hasPermission("MANAGE_MESSAGES")) return message.reply("**ما عندي برمشن**").then(msg => msg.delete(6000))
  ...
  if(!toMute) return message.channel.sendMessage("**منشن لـ الي معمولو ميوت**:x: ");
  ...
  message.channel.sendMessage("**لقد تم فك الميوت بنجاح**:white_check_mark:");
...
client.on('message', message => {
  ...
  let user = message.mentions.users.first();
  let reason = message.content.split(" ").slice(2).join(" ");
  if (message.mentions.users.size < 1) return message.reply("**منشن للشخص**");
...
client.on('message', message => {
   if(message.channel.type === "dm") return;
     ...
     if(message.mentions.users.size > 1) return message.reply('مينفعش ولد ي شاذ').catch(console.error);
      if(proposed === message.author) return message.reply(`**خونتني**`);
       if(proposed === client.user) return message.reply(`**تتجوزيني **`);
